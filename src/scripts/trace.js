...
function hopRow(h, i, total) {
  ...
  const to = h.location
    ? `<div class="hop-to"><span class="hop-arrow" aria-hidden="true">&#8595;</span> <a href="${escAttr(h.location)}" target="_blank" rel="noopener noreferrer">${escHtml(h.location)}</a></div>`
    : '';
  ...
  return `<div class="hop${settled ? ' hop--final' : ''}">
    ...
    <div class="hop-body">
      <div class="hop-line">
        ...
        <a class="hop-url" href="${escAttr(h.url)}" target="_blank" rel="noopener noreferrer">${escHtml(h.url)}</a>
      </div>
      ${to}
      ${finalTag}
    </div>
